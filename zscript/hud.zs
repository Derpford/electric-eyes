class EEHud : BaseStatusBar {
    ...
    void DrawFullscreenStuff() {
        ...
        if (plr) {
            ...
            DrawBar(hbar,"HBARE0",hp,maxhp,(80,-40),0,0,lbarflags,max(hpperc**2,hpnoise));

            // And armor.
            DrawBar("ABARA0","HBARE0",arm,maxarm,(80,-40),0,0,lbarflags);
            
            // Draw the herb kit status.
            DrawBar("HRBBA0","HRBBB0",herbcharge,25.,(80,-34),0,0,lbarflags);
            for (int i = 0; i < healcount; i++) {
                double xofs = i * 18;
                DrawImage("CROSA0",(178+xofs,-40),lbarflags);
            }

            // Draw ammunition.
            DrawString(mNumberFont,String.format("%03d",mag),(80,-72),lbarflags,Font.CR_WHITE);
            DrawString(mConFont,String.format("%03d",ammo),(112,-72),lbarflags,Font.CR_WHITE);
			...
			for(int i = 0; i < 6; i++)
			{
				if(plr.CheckKeys(i+1,false,true)) { 
                    DrawImage(keySprites[i],(80+(16*i),-16),lbarflags,scale:(2,2)); 
				}
			}
        }
    }
